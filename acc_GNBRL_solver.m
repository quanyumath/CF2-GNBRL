function [A, L, S] = acc_GNBRL_solver(X, maxit, Beta, alpha, rho, lambda, r, A, PSI)
...
% if r < n2
%     [U, D, V] = tsvd(X, 'econ');
%     A = tprod(tprod(U(:, 1:r, :), D(1:r, 1:r, :)), tran(V(1:r, 1:r, :)));
% elseif r == n2
%     A = D;
% else
%     A = adjust_r(X, [n1, r, n3]);
% end
hat_A = A;
A0 = A;
hat_L = L;
...
%        [C{u}, ~, ~] = prox_ttnn_psi(C{u}, nabla_ATu, alpha(u)/beta(u), psi);
    end
...
%     L = prox_ttnn_psi(L, Lnabla_f, lambda(1)/L_L/Beta, psi);
    ...
    for u = 1:3
        ...
        beta(u) = min(rho*beta(u), 1e8);
    end
    Beta = min(1.3*Beta, 1e8);
...
%    w = 0;  %  no acceleration
    ...
    cgamma = max((gamma-1) / 2 / gamma, 0.9^t);
    ...
    if vartheta2 < 1e-3
        ...
        % Compute vartheta_A
        DT = diffH3(T,[n1,n2,n3]); nabla_fA = tprod(tprod(A, L)+S-X, tran(L));
